using System;
using System.Collections.Generic;

#nullable disable

namespace GiftForMyLove.Models
{
    public partial class InxVanPhong
    {
        public double? Stt { get; set; }
        public string Id { get; set; }
        public string MaNsOld { get; set; }
        public string MaNsNew { get; set; }
        public string TenNv { get; set; }
        public string MaBp { get; set; }
        public DateTime? NgaySinh { get; set; }
        public string SoCmnd { get; set; }
        public string DiaChiTt { get; set; }
        public string DiaChiHt { get; set; }
        public string NgayCapCmnd { get; set; }
        public string NoiCap { get; set; }
        public string SoHc { get; set; }
        public string NgayCapHc { get; set; }
        public string NoiCapHc { get; set; }
        public string Email { get; set; }
        public DateTime? NgayBd { get; set; }
        public DateTime? NgayKt { get; set; }
        public string MaDv { get; set; }
        public double? GioiTinh { get; set; }
        public double? LanhDao { get; set; }
        public string MaChucVu { get; set; }
        public string DanToc { get; set; }
        public string TonGiao { get; set; }
        public string Qt { get; set; }
        public string MaAnh { get; set; }
        public string NguyenQuan { get; set; }
        public string HandPhone { get; set; }
        public string MaSoThue { get; set; }
        public string TinhTrangHn { get; set; }
        public string NoiSinh { get; set; }
        public string TdVh { get; set; }
        public string TdHv { get; set; }
        public string TdNn { get; set; }
        public string TdTh { get; set; }
        public string ChuyenMon { get; set; }
        public string CvChinh { get; set; }
        public string SoTkNh { get; set; }
        public string TenNh { get; set; }
        public bool TrangThai { get; set; }
        public string LyDoNghi { get; set; }
        public string SoQdNghi { get; set; }
        public string MaMayCc { get; set; }
        public string MaCa { get; set; }
        public bool? KhongDvt { get; set; }
        public double? NguoiPt { get; set; }
        public string MaVt { get; set; }
    }
}
